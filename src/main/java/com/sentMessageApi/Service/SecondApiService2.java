...
@Service
public class SecondApiService2 {

	@Value("${paisaUserName}")
	private String paisaUserName;
	@Value("${user.shorturl}")
	private String paisaShorturl;
	Logger logger = LoggerFactory.getLogger(SecondApiService2.class);
				...
				if (mDetails != null) {
					...
					if (trackingIdList.isEmpty()) {
						...
						if (authTokenRes!=null) {
							...
							if (longUrlDtoResponse.getData() != null) {
								...
								for (Map.Entry<String, String> entry : trackingIDVsLongUrl.entrySet()) {
									...
									if (shortUrlDtoeEntity != null) {
										...
										logger.info("Unable to create Short Url  of BlacklistMsisdn"+ fData.getBlacklistMsisdn()+ " campaignName: "+ fData.getCampaignName()+ " Username: "+ fData.getUserName());
										fData.setFlag(1);
								...
								logger.info("No longUrl data found for this user BlacklistMsisdn"+ fData.getBlacklistMsisdn()+ " campaignName: "+ fData.getCampaignName()+ " Username: "+ fData.getUserName() );
								fData.setFlag(1);
								faRepository.save(fData);
								continue;
							}
							...
							logger.info("Unable to get jwttoken for this user BlacklistMsisdn"+ fData.getBlacklistMsisdn()+ " campaignName: "+ fData.getCampaignName()+ " Username: "+ fData.getUserName() );
							fData.setFlag(1);
							faRepository.save(fData);
							continue;
						}
...
//						 4. call userApi At last with fdata campaignId, templateValue, ukey
						...
						Map<String, String> Resreq = restApiCall.SecondapiCall(fData, "chat",campaignId, templateValue, ukey);
						if (Resreq != null) {
//							logger.info("Saving request and response data to second api table !!");
							this.saveReqRes(Resreq.get("req"), Resreq.get("res"), fData);
						}
						else {
							logger.info("Getting Exception While Calling /generateToken: " + fData);
							fData.setFlag(1);
							faRepository.save(fData);
						}
